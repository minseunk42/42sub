...
int exec_cmd(t_list *cmd, char **env)
{
	...
	else if (pid == 0)
	{
		...
		if (cmd->previous && cmd->previous->type == TYPE_PIPE
			...
			return (exit_fatal());
		if ((out = execve(cmd->args[0], cmd->args, env)) < 0)
		{
...
int executor(t_list **cmds, char **env)
{
	t_list	*crt;
	int		out;
	...
	while (*cmds)
	{
		...
		if (strcmp("cd", crt->args[0]) == 0)
		{
			out = EXIT_SUCCESS;
			if (crt->length < 2)
				out = show_error("error: cd: bad arguments\n");
			else if (chdir(crt->args[1]))
			{
				out = show_error("error: cd: cannot change directory to ");
				show_error(crt->args[1]);
				show_error("\n");
			}
		}
		else
			out = exec_cmd(crt, env);
		if (!(*cmds)->next)
			break ;
		*cmds = (*cmds)->next;
	}
...
int main(int argc, char **argv, char **env)
{
	...
	if (cmds)
	{
		if (executor(&cmds, env) && free_all(&cmds))
			return (EXIT_FAILURE);
	}
	free_all(&cmds);
	return (EXIT_SUCCESS);
